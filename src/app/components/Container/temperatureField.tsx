...
export const TemperatureField = observer(() => {
    ...
    const windowWHRatio = window.innerWidth + window.innerHeight;

    /**
     * 캔버스의 가로 축을 정확히 기온 배열의 최대값만큼 나눠 나란히 전개합니다.
     * @param days
     */
    const getLinePositionX = useCallback(
        (days: number) => {
     ...
     * @param temperature
     */
    const getLinePositionY = useCallback(
        (temperature: number) => {
            const canvas = canvasRef.current!;
    ...
    const getLines = useCallback(
        (temperatures: number[]): Array<Line> => {
        ...
        [getLinePositionX, getLinePositionY]
    );

    /**
     * 무한 스크롤링을 위한 캔버스 이미지 패턴을 생성합니다.
     */
    const drawChartPattern = useCallback(
        (pattern: { valid: boolean; src: string }) => {
    ...
    const renderFrame = useCallback(
        ...
        [chartMembers, config, drawChartPattern, drawLines, renderOK]
    );
    ...
    }, [renderOK, windowWHRatio, renderFrame]);
